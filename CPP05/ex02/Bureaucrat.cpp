...
void Bureaucrat::signForm(AForm& form) const {
    try {
        form.beSigned(*this);
        std::cout << _GREEN << getName() << " signed " << form.getName() << _END << std::endl;
...
void Bureaucrat::executeForm(AForm const& form) const {
    try {
        form.execute(*this);
        std::cout << _GREEN << getName() << " executed " << form.getName() << _END << std::endl;
    } catch (std::exception& e) {
        std::cout << _RED << getName() << " couldn't execute " << form.getName()
                  << " because " << e.what() << _END << std::endl;
    }
}
